...
### Phase 0 — 프로젝트 셋업 (1주)
- [ ] Flutter 프로젝트 생성, 5개 플랫폼 빌드 확인
- [ ] 폴더 구조(Clean Architecture) + Riverpod/드리프트 스캐폴딩
- [ ] CI(GitHub Actions): 포맷·분석·테스트, 플랫폼별 빌드
- [ ] 라이선스/앱 아이콘/이름 확정

### Phase 1 — 터미널 코어 (2주)
- [ ] `xterm.dart` 터미널 위젯 통합, 리사이즈/스크롤백
- [ ] `dartssh2`로 비밀번호 인증 SSH 접속
- [ ] PTY 연결: 키 입력 → SSH, 출력 → 터미널 렌더링
- [ ] 연결 상태(연결중/실패/종료) 처리 및 에러 표시
- **완료 기준:** 임시 호스트 정보로 실제 서버 접속·명령 실행
