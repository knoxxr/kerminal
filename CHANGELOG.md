...
### Fixed
- **공유 초대가 "계정이 없습니다"로 실패하던 문제** — 가입할 때 이메일에 대문자가
  하나라도 있으면(`Foo@example.com`) 프로필이 그대로 저장돼, 동료가 소문자로 검색해도
  **영구히 찾을 수 없었습니다.** 저장·검색 양쪽을 소문자로 정규화했고, 이미 저장된
  행도 스키마 재실행 시 정리됩니다.
- **변경 이력 되돌리기가 영구히 깨질 수 있던 문제** — 두 기기가 동시에 저장하면 같은
  버전 번호가 두 번 기록되고, 그러면 그 버전으로의 롤백이 계속 실패했습니다. 번호
  충돌 시 다시 읽어 재시도하도록 하고, DB에 중복 방지 제약을 추가했습니다(기존 중복은
  **삭제하지 않고 빈 번호로 이동**시켜 이력을 보존합니다).
- **인증 방식을 바꿔도 이전 비밀이 남던 문제** — 비밀번호 방식 서버를 SSH 키로
  바꾸면서 키를 넣지 않으면, 쓰이지 않는 비밀번호가 저장소에 계속 남고 접속은 빈 키로
  시도됐습니다.
- **저장된 인증 방식 값이 범위를 벗어나면 서버 목록 전체가 깨지던 문제** (구버전으로
  내려갔을 때) — 기본값으로 안전하게 처리합니다.

### Changed
- **호스트 키 검증이 없으면 접속을 거부합니다** — 이전에는 검증자가 없으면 **모든 키를
  수락**하는 것이 기본이었습니다. 현재 모든 접속 경로가 검증자를 넘기지만, 기본값이
  안전한 쪽이어야 나중에 빠뜨렸을 때 조용히 중간자 공격에 노출되지 않습니다.
- 로컬 DB에 **마이그레이션 전략**을 선언했습니다. 없으면 다음에 스키마를 바꾸는 순간
  기존 설치가 DB를 열지 못해 앱이 시작되지 않습니다.
- CI가 **모든 PR**을 검증합니다(이전에는 `main`을 향한 PR만 — 브랜치 간 PR이 검증
  없이 통과할 수 있었습니다).
...
- **파일 전송(SFTP)** — 터미널 상단의 폴더 아이콘을 누르면 **접속된 그 연결로** 서버
  파일을 탐색할 수 있습니다. 새로 로그인하지 않으므로 2단계 인증 코드를 다시 넣을
  필요가 없습니다.
  - 폴더를 눌러 이동, 경로 표시줄의 각 단계를 눌러 상위로 바로 이동, ↑ 로 한 단계 위.
  - 파일을 누르면 **내려받기**(데스크톱은 저장 위치 선택, 모바일은 앱 문서 폴더에
    저장하고 경로를 알려줍니다), 상단 ⬆ 로 **올리기**.
  - 이름 바꾸기, 새 폴더, 삭제. 파일 크기와 수정 시각을 함께 보여줍니다.
  - **폴더 삭제는 빈 폴더만** 가능합니다 — 재귀 삭제는 한 번의 실수로 서버 디렉터리를
    통째로 날릴 수 있고 SFTP에는 되돌리기가 없어서 의도적으로 넣지 않았습니다.
- **`~/.ssh/config`에서 서버 가져오기 (데스크톱)** — 호스트 목록의 ⋮ 메뉴 →
  **Import from ~/.ssh/config**. 이미 쓰던 서버 수십 개를 손으로 입력하지 않아도
  됩니다.
  - 가져올 항목을 **미리 보고 선택**합니다. 이미 저장된 것(같은 주소·포트·계정)은
    기본으로 해제돼 있어 두 번 실행해도 중복되지 않습니다.
  - **비밀번호·개인키는 디스크에서 읽지 않습니다.** 설정 파일은 키 파일 경로만
    가리키고, 사용자 동의 없이 개인키를 읽는 것은 옳지 않기 때문입니다. 첫 접속 때
    입력하시면 됩니다.
  - `Host *` 같은 **패턴은 건너뜁니다**(다른 항목의 기본값이지 접속 대상이 아닙니다).
  - `ProxyJump`가 필요한 항목은 **"아직 지원하지 않음"으로 표시**합니다 — 그냥
    가져오면 접속되지 않는 서버가 생깁니다.
  - 가져온 서버는 `Imported` 그룹으로 들어갑니다.
  - 모바일은 `~/.ssh`에 접근할 수 없어 메뉴에 표시되지 않습니다.
- **2단계 인증(OTP) 서버에 접속할 수 있습니다** — 지금까지는 키보드 인터랙티브
  인증을 처리하지 않아, 일회용 코드를 요구하는 서버에는 **접속 자체가 불가능**했습니다
  (인증 실패로만 보였습니다). 이제 서버가 묻는 항목을 그대로 보여주는 입력창이 떠서
  코드를 입력할 수 있습니다. 비밀번호 만료로 새 비밀번호를 요구하는 경우도 같은
  경로로 처리됩니다.
  - 서버가 보낸 프롬프트대로 입력칸이 만들어지고, 비밀 항목은 가려집니다.
  - 재접속·세션 복제 시에도 같은 방식으로 다시 인증합니다.
...
- **동기화가 조용히 영구 정지되던 문제** — 클라우드 호스트 중 **하나라도** 복호화에
  ...
  오프라인"과 구분 없이 삼켜져 사용자가 알 수 없었습니다. 이제 문제가 있는 호스트만
  건너뛰고, 실패는 목록 상단에 표시됩니다.
- **여러 변경이 동시에 도착할 때 동기화가 겹쳐 실행되던 문제** — 실시간 이벤트가
  행마다 발생해 reconcile이 중첩되고, 같은 호스트를 중복 업로드할 수 있었습니다.
  한 번에 하나만 실행하도록 직렬화했습니다.
- **macOS: 저장된 비밀번호·키가 사라질 수 있던 문제** — 볼트 초기화에 잠금이 없어
  동시 호출 시 크래시하거나 암호화 키를 덮어써 기존 데이터를 읽을 수 없게 될 수
  있었습니다. 또한 복호화에 실패하면 곧이어 **원본 파일을 빈 값으로 덮어써** 복구가
  불가능했습니다. 이제 초기화를 단일화하고, 읽을 수 없는 파일은
  `vault.enc.unreadable`로 **보존**한 뒤 새로 시작합니다. 볼트 파일 권한도 600으로
  제한합니다.
- **잘못된 패스프레이즈로 계정 암호화 키가 재생성되던 문제** — 계정에 암호화 키가
  없는 상태에서는 **어떤 패스프레이즈를 넣어도** 새 키가 만들어지고 공개키가
  덮어써져, 이미 동기화·공유된 모든 호스트가 영구히 복호화 불가가 됐습니다. 이제
  "키 없음"과 "패스프레이즈 틀림"을 구분하고, 키 생성은 **결과를 경고한 뒤 확인을
  받는 별도 동작**으로 분리했습니다.
- **악성 백업 파일로 앱을 멈출 수 있던 문제** — 백업의 `iterations` 값을 그대로
  신뢰해, 큰 값을 넣은 파일을 열면 앱이 사실상 정지했습니다. 상한을 두고 알고리즘
  필드도 검증합니다.
- **알려진 호스트 목록이 손상되면 접속 자체가 불가능해지던 문제** — 저장소가 깨졌을 때
  예외가 호스트 키 검증 경로로 전파됐습니다. 이제 "모두 처음 보는 호스트"로 처리해
  지문을 다시 확인받습니다.
### Changed
- **UI를 "버튼만 봐도 아는" 방향으로 다듬었습니다.**
  - **접속이 한 번 탭으로 됩니다** — 이전에는 **더블탭**만 접속이어서, 한 번 눌러도
    아무 일이 없어 앱이 고장난 것처럼 보였습니다. 게다가 목록에 "접속" 버튼이 아예
    없었습니다. 이제 각 서버 행에 **▶ 버튼**이 보이고, 행을 한 번 탭해도 접속됩니다.
  - **"최근 삭제"의 휴지통 아이콘을 없앴습니다** — 상단바의 🗑 아이콘이 "삭제"로
    읽혀 오해를 줬습니다(실제 기능은 복원). 이제 ⋮ 메뉴의
    **"Restore a deleted server"** 로 들어갑니다.
  - **＋ 버튼에 라벨을 붙였습니다** — "Add server".
  - **빈 목록에서 두 선택지의 차이를 설명합니다** — "Add a server"(저장해두고 한 번에
    접속) vs "Connect without saving"(한 번만). 이전에는 "Quick Connect"가 무슨
    뜻인지 알 수 없었습니다.
  - **터미널 툴바 키에 설명을 붙였습니다** — `^C` → `Ctrl+C`, 길게 누르면 "Stop the
    running command" 같은 설명이 나옵니다.
  - **계정 화면에서 비밀번호 두 개의 차이를 명시했습니다** — "Account password
    (로그인용)"와 "Encryption passphrase(기기에서 서버 정보를 암호화, 분실 시 복구
    불가)". 잠금 해제 버튼도 "Unlock" → **"Decrypt my servers"**.
  - **설정 문구 정리** — "Backup & Share" → **Backup**(호스트 공유와 혼동 제거),
    "Encrypted export" → **"Save to file…"**, "Import" → **"Restore from file…"**.
  - 서버 추가 화면: 긴 개인키 입력 때문에 저장 버튼이 화면 밖으로 밀리던 문제 →
    **상단바에도 저장 버튼**을 두었습니다. 라벨도 'Label'→'Name', 'Host'→'Address'로
    바꾸고 예시를 넣었습니다.
  - 삭제 확인 대화상자가 **무엇이 함께 지워지는지**(저장된 비밀번호·키)와 **복원
    경로**를 알려줍니다.
  - 용어를 'host' → **'server'** 로 통일했습니다(일반 사용자에게 'host'는 모호함).

## [0.6.0]

### Fixed
- **안드로이드에서 신버전으로 업데이트되지 않던 문제** — 지금까지의 릴리스 APK는
  서명 키가 CI에 없어 **디버그 키로 서명**되었고, 그 키가 **빌드마다 달랐습니다.**
  안드로이드는 서명이 다른 APK로 덮어쓰기를 막기 때문에(`INSTALL_FAILED_UPDATE_INCOMPATIBLE`)
  설치는 되지만 업데이트가 되지 않았습니다. 고정된 릴리스 키로 서명하도록 바꿨고,
  서명 키가 없으면 릴리스 빌드가 실패하도록 해 재발을 막았습니다.

  > ⚠️ **이번 한 번은 제거 후 재설치가 필요합니다.** 서명이 바뀌므로 기존 앱 위에
  > 덮어쓸 수 없습니다. 저장된 호스트가 사라지니 **설정 → Encrypted export**로
  > 백업한 뒤 새로 설치하고 가져오기 하세요. 0.6.0 이후 버전끼리는 정상적으로
  > 업데이트됩니다.
...
### Added
- **안드로이드: 앱 전환에도 세션이 그대로 유지됩니다** — 재연결이 아니라 **소켓 자체를
  유지**하므로, 다른 앱에 갔다 와도 실행 중이던 명령과 셸 상태가 이어집니다. 세션이
  하나 이상 열리면 포그라운드 서비스가 프로세스를 붙잡아 두고 "세션 N개의 연결을
  유지하고 있습니다" 알림이 상주합니다. 마지막 탭을 닫거나 앱을 스와이프로 종료하면
  알림도 사라집니다.
  - 알림을 탭하면 앱으로 돌아옵니다.
  - 안드로이드 13+에서는 알림 권한을 한 번 묻습니다. **거부해도 연결 유지는 그대로
    동작**하고 알림만 보이지 않습니다.
  - iOS는 OS가 프로세스를 정지시켜 유지가 원천적으로 불가능합니다 — 복귀 시 자동
    재연결(0.4.9)로 동작합니다.
  - 안드로이드가 메모리 압박으로 액티비티까지 파괴한 경우에는 여전히 탭이 사라질 수
    있습니다(Flutter 엔진 캐싱은 후속 과제). 그 경우에도 0.4.9의 복귀 시 자동 재연결이
    안전망으로 남아 있습니다.
